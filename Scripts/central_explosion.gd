...
var size = 1  # explosion size for all directions
const TILE_SIZE = 16

var animation_names = ["up", "right", "down", "left"]
var animation_directions: Array[Vector2] = [
	Vector2(0, -TILE_SIZE), Vector2(TILE_SIZE, 0),
	Vector2(0, TILE_SIZE), Vector2(-TILE_SIZE, 0)
]
...
func create_explosion_for_size(size: int, animation_name: String, animation_position: Vector2):
	for i in size:
		if i < size - 1: # will create middle animations until collision or reach max size
...
func calculate_size_of_explosion(raycast: RayCast2D):
	...
	if collider is TileMapLayer:
		...
		var size_of_explosion_before_collider = max(roundi(distance_to_collider / TILE_SIZE) - 1, 0)
		return size_of_explosion_before_collider

func execute_explosion_collision(collider):
